...
`/new-chrome-extension <name>` で `H:/ClaudeCode/GoogleChrome/<name>/` に Chrome 拡張の雛形を一括生成する。
...
`H:/ClaudeCode/GoogleChrome/$ARGUMENTS/` が既に存在する場合は **上書きせず中断**して、ユーザーに別名を提案する。
...
foreach ($size in 16, 48, 128) {
    ...
    # ... 中央に文字描画 ...
    $bmp.Save("H:\ClaudeCode\GoogleChrome\<name>\icons\icon$size.png", [System.Drawing.Imaging.ImageFormat]::Png)
    $bmp.Dispose()
}
```
...
### 6. ルート CLAUDE.md のサブディレクトリ一覧を更新

`H:/ClaudeCode/GoogleChrome/CLAUDE.md` の「サブディレクトリ」セクションに新拡張へのリンクを追加する。
...
2. デベロッパーモードをオン
3. 「パッケージ化されていない拡張機能を読み込む」→ H:\ClaudeCode\GoogleChrome\<name>\ を選択
4. 対象ページで動作確認
```
